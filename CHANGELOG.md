...
All notable changes to Icon Pro Max are documented here.
...
### Added
- Initial release: complete icon catalog for the Personal Web (example.com).
- **Payment methods** — VietQR, VNPAY, ZaloPay, MoMo, Visa, Mastercard with the
  shared badge wrapper and per-logo canonical heights.
- **QR codes** — static MoMo / ZaloPay images and the dynamic VietQR `qrUrl()`
  recipe.
...
- **Site & brand** — favicons, PWA icons, Apple touch icon, BIMI, OG image,
